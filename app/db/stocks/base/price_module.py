...
class PriceFetcher:
    ...
    def _get_price(
        ...
        if data_source == "yahoo finance":
            prices = yfinance.download(symbols, start=start, end=end, interval=interval)
            if len(symbols.split(" ")) == 1:
                prices["symbol"] = symbols
            else:
                prices = prices.stack()
            prices = standardize_dataframe_column(
                prices, {"level_1": "symbol", "Datetime": "date"}
            )
            ...
            if interval != "1day":
                ...
                for symbol in symbols.split(" "):
                    url = urljoin(
                        base_url, quote(f"historical-chart/{interval}/{symbol}")
                    )
                    try:
